    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100">Send Notification</h3>
          <button
          ...
          <form onSubmit={handleSendNotification} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-amber-500 dark:focus:ring-amber-600"
                required
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-amber-500 dark:focus:ring-amber-600 resize-none"
                rows="3"
                required
              />
            </div>
      ...
      {/* Notifications List */}
      <div className="rounded-xl border border-slate-200 dark:border-slate-700 p-0 overflow-hidden">
        <div className="sticky top-0 z-10 bg-white/90 dark:bg-slate-900/90 backdrop-blur border-b border-slate-200 dark:border-slate-700 px-6 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="flex items-center gap-2">
            <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100">Recent Notifications</h3>
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-200 border border-amber-200 dark:border-amber-700">
              {unreadCount} unread
            </span>
          </div>
          <div className="flex items-center gap-2 sm:justify-end">
            <button onClick={markAllAsReadNow} className="text-sm text-amber-700 dark:text-amber-200 hover:text-amber-800 dark:hover:text-amber-100 font-medium">Mark all as read</button>
            {hasMore && (
              <button onClick={loadMore} className="text-sm text-amber-700 dark:text-amber-200 hover:text-amber-800 dark:hover:text-amber-100 font-medium">Load older</button>
            )}
          </div>
        </div>

        {notifications.length === 0 ? (
          <div className="py-10 text-center text-slate-500 dark:text-slate-400">No notifications yet.</div>
        ) : (
                            ...
                            <div className="flex flex-col gap-2 items-end">
                              {!isRead(notification) && (
                                <button
                                  ...
                                  className="px-3 py-1 rounded-lg border border-amber-300 dark:border-amber-700 text-amber-700 dark:text-amber-200 hover:bg-amber-50 dark:hover:bg-amber-900/20 text-xs font-medium"
                                >
                                  Mark read
                                </button>
                              )}
                              ...
                              {isSender(notification) ? (
                                ...
                                <div className="flex flex-col gap-1 w-full items-end">
                                  <button
                                    ...
                                    className="px-3 py-1 border border-amber-200 dark:border-amber-700 text-amber-700 dark:text-amber-200 rounded hover:bg-amber-50 dark:hover:bg-amber-900/20 text-xs font-medium"
                                  >
                                    Hide
                                  </button>
                                  <button
      ...
      {/* Sent by You */}
      <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6">
        <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-4">Sent by You</h3>
        {sent.length === 0 ? (
          ...
          <div className="space-y-3 overflow-y-auto pr-1" style={{ maxHeight: "520px" }}>
            {sent.map((n) => {
              ...
              return (
                <div key={n._id} className="border border-slate-200 dark:border-slate-700 rounded-lg p-4 bg-white dark:bg-slate-900">
                  {isEditing ? (
                    <form onSubmit={handleEditNotification} className="space-y-3">
                      <input
                        type="text"
                    ...
                    <>
                      <div className="flex justify-between items-start gap-3">
                        <div className="flex-1 min-w-0 space-y-1">
                          <h4 className="font-medium text-slate-800 dark:text-slate-100 truncate">{n.title}</h4>
                        ...
                        <button
                          ...
                          className="px-3 py-1 text-xs font-semibold rounded-lg border border-amber-200 dark:border-amber-700 text-amber-700 dark:text-amber-200 hover:bg-amber-50 dark:hover:bg-amber-900/20"
                        >
      ...
      <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6">
        ...
        {(hiddenList || []).length === 0 ? (
          ...
          <p className="text-slate-600 dark:text-slate-300 text-sm">Hidden notifications are collapsed. Click Show to view them.</p>
        ) : (
          <div className="space-y-3 overflow-y-auto pr-1" style={{ maxHeight: "360px" }}>
            {hiddenList.map((n) => (
              <div key={n._id} className="border border-slate-200 dark:border-slate-700 rounded-lg p-4 bg-slate-50 dark:bg-slate-800/50">
                ...
                <p className="mt-2 text-sm text-slate-700 dark:text-slate-300 leading-relaxed whitespace-pre-wrap">{n.message}</p>
              </div>
            ))}
          </div>
        )}
